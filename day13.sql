...
-- 2. ROWNUM : 조회된 경과의 첫번쨰 행부터 1로 증가하는 값
---------------------------------------------------
...
-----------------------------------------------------------------
-- ORDER BY 절에서 ROENUM 이 변경되는 결과 확인
...
----------------------
    ...
    5	7521	WARD
*/
-- => ROWNUM 이 ORDER BY 결과에 영행을 받지 않는 것처럼 보일 수 있음.
--    SUB-QUERY 로 사용할 때 영향을 받음

-- (4) SUB-QUERY 를 썼을 때 ROWNUM 의 값 확인
SELECT ROWNUM
     , a.empno
     , a.ename
     , a.numrow
...
-- 이름에 A 가 들어가는 사람을 이름순으로 정렬하여 조회
SELECT ROWNUM
     , e.ename
  FROM emp e
 WHERE e.ename LIKE '%A%'
...
;
/*
4	AAAR9lAAHAAAADGAAJ	JAMES
3	AAAR9lAAHAAAADGAAD	JONES
1	AAAR9lAAHAAAADEAAB	J_JAMES
2	AAAR9lAAHAAAADGAAA	SMITH
*/

-- 이름에 S 가 들어가는 사람들의 조회 결과를 SUB-QUERY로 감쌌을 때의 ROWNUM, ROWID 확인
SELECT ROWNUM
     , a.rowid
     , a.ename
  FROM (SELECT e.rowid
...
;
/*
1	AAAR9lAAHAAAADGAAJ	JAMES
2	AAAR9lAAHAAAADGAAD	JONES
3	AAAR9lAAHAAAADEAAB	J_JAMES
4	AAAR9lAAHAAAADGAAA	SMITH
*/

-- ROWNUM 으로 할 수 있는 쿼리
-- EMP 에서 급여를 많이 받는 상위 5명으로 조회하시오
...
SELECT e.empno
 ...
 ORDER BY e.sal DESC
;
-- 2. 1의 결과를 SUB-QUERY 로 FROM 절에 사용하여 ROWNUM 을 같이 조회
SELECT ROWNUM
     , a.*
  FROM (SELECT e.empno
             , e.ename
...
--      : VALUES 정레 반드시 전체 컬럼의 데이터를 순서대로 모두 나열
INSERT INTO MEMBER 
...
INSERT INTO MEMBER 
VALUES ('M003', '이변현', '0186', sysdate, NULL, NULL, 3, 'M');
...
위처럼 VALUES 절에 나열된 값의 개수가 전체 컬럼 수와 일치하지 않으면 아래와 같은 오류가 발생하므로
NULL 입력을 해서라도 값의 개수를 맞추어야 한다.
...
--      : AVLUES 절에 INTO 의 순서대로 값의 타입, 갯수를 맞추어서 작성
...
-- INTO 절과 VALUES 절의 갯수는 같으나 값의 타입이 일치하지 않을 때
-- 숫자 데이터 컬럼인 BIRTH_MONTH 에 '한양' 이라는 문자를 추가하려는 시도
...
-- 문제) NEW_MEMBER 테이블에 MEMBER 테이블로부터 데이터를 복사하여 다중행 입력을 하시오
--       단, MEMBER 테이블의 데이터에서 BIRTH_MONTH 가 홀수달인 사람들만 조회하여 입력하시오
...
--              WHERE 조건절의 조합에 따라서 1행만 수정하거나 다중 행 수정 가능
...
UPDATE member m -- 테이블 별칭
 ...
 WHERE m.member_id = 'M006' -- PK로 찾아야 정확히 홍길동 행울 찾을 수 있음
;
COMMIT;
/*
1 행 이(가) 업데이트되었습니다.
...
--      초기 INSERT 시 NULL 로 데이터를 받지 않은 경우 후에 데이터 수정이 발생할 수 있다.
...
-- 예) 유현동(M009) 멤버의 전공을 수정 / 역문컨
UPDATE member m
   SET m.major = '영문컨'
-- WHERE m.member_id = 'M009' -- 실수로 WHERE 누락
;
-- 9개 행 이(가) 업데이트되었습니다.
-- where 조건절 누락 때문에 모든 행에 대해서 major 컬럼이 모두 수정이 일어난 결과

-- => DML 작업 실수, 주의 점: UPDATE 구문 오류는 아니라는 점

-- COMMIT 작업까지 되돌리는 ROOLLBACK 명령으로 잘못된 업데이트 되돌리기
ROLLBACK;
-- 롤백 완료.
-- M004 멤버의 전화번호를 업데이트 한 것이 마지막커밋이므로 그 상태의 데이터로 복원

-- 예) 정확한 유현동(M009) 멤버의 전공을 수정 / 역문컨
UPDATE member m
   SET m.major = '영문컨'
...
UPDATE member m
   SET m.gender = 'M'
 WHERE m.address = '안양시'
 -- WHERE 조건절(X), WHERE 조건절 문법 오류(X)
;
-- 1 행 이(가) 업데이트되었습니다.
ROLLBACK;
-- 롤백 완료.
-- 위의 실행 결과는 얼핏 정상작동한 것 처럼 보이지만 데이터가 다양해지면 오작동의 여지가 있는 구문이다.
-- 따라서 UPDATE 작성시에는 WHERE 조건절 작성시 주의를 기울여야 함.

-- 1행을 수정하는 목적이라면 반드시 PK 컬럼을 비교해야 한다.
-- PK 컬럼은 전체 행에서 유일하고 NOT NULL 이 보장되는 컬럼이므로
-- 반드시 그 행을 찾을 수 있는 값이기 때문에 PK 사용이 권장됨


-- UPDATE 구문에 SELECT 서브쿼리 사용
-- 예) 김지우(M008) 멤버의 major를 오진오(M002) 멤버의 major 로 수정

-- 1) M008의 major 를 구하는 SELECT
SELECT m.major
  FROM member m
 WHERE m.member_id = 'M008'
;

-- 2) (1) 의 결과를 major 를 수정하는 UPDATE 구문 작성
UPDATE member m
   SET m.major = ?
...
-- 만약 SET 절에 사용하는 서브쿼리의 결과가 정확하게 1행 1열의 1개의 값이 아니면 구문 오류
UPDATE member m
   SET m.major = (SELECT m.major
                    FROM member m)
 WHERE m.member_id = 'M002'
...
-- 예) NOT NULL 인 member_name 에 NULL 데이터로 업데이트를 시도하는 경우
UPDATE member m
...
-- 예) M001 데이터에 대해서 birth_month를 -1 로 수정시도
UPDATE member m
   SET m.birth_month = -1
 WHERE m.member_id = 'M001'
;
...
------------------------------------------------------------------------
-- 수업중 실슴
-- 1) 현재 데이터 상태에서  PHONE 컬럼이 NULL 인 사람들은 일괄적으로 '0000'으로 업데이트
--  : PK 필요없는 구문
UPDATE member m
   SET m.phone = '0000'
...
-- 3) ADDRESS 컬럼이 NULL 인 사람은 일괄적으로 '아산시'로 업데이트
--  : PK 필요없는 구문
UPDATE member m
   SET m.address = '아산시'
