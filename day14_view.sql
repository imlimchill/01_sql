...
--- (4) 사용자 평접
--- (5) 시스템 권한 탭
--- (6) CREATE VIWE 권한이 부여됨 체크박스 선택 -> 적용
-- 6개의 작업을 한 줄로 코드로 나타내면
...
-- 2. 복사 테이블에는 pk 설정이 누락되어 있으므오 pk 설정을 alter로 추가
/*
    ...
    new_emp : PK_NEW_EMP, empno 컬럼을 PRIMARY KEY 로 설정
              ...
              FK_MGR, mgr 컬럼을 FOREIGN KEY 로 설정
                 new_emp 테이블의 empno 컬럼으 REFERENCES 하도록 설정
*/


-- 3. 복사테이블에서 기본 테이블로 하는 VIEW 를 생성
